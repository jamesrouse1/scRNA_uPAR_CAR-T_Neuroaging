...
run_psychad <- function() {
    ...
    obj <- read_h5ad_as_seurat(input_file, file.path(outdir, "PsychAD_seurat.rds"))
    obj <- score_human(obj)
    ...
    if (anyNA(c(condition_col, class_col, age_col))) 
        stop("PsychAD published condition, cell class or age metadata were not retained")
    ...
    stats <- make_split_violin(pvm, "plaur_alra", "condition_plot", c("Control", "AD"), "ALRA-imputed PLAUR in PsychAD PVMs", 
        "ALRA-imputed PLAUR expression", file.path(outdir, "Figure_2S_PLAUR_PVM_Control_AD.pdf"), c("#4C78A8", 
            "#E45756"))
