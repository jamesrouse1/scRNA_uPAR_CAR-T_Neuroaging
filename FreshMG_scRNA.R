...
run_freshmg <- function() {
    ...
    obj <- read_h5ad_as_seurat(input_file, file.path(outdir, "FreshMG_seurat.rds"))
    obj <- score_human(obj)
    ...
    if (anyNA(c(age_col, class_col))) 
        stop("FreshMG published age/cell annotations were not retained")
    ...
    if (!length(young) || !length(old)) 
        stop("FreshMG young/old labels were not recognized")
    pvm$age_group <- ifelse(pvm$age_plot %in% young, "Young", ifelse(pvm$age_plot %in% old, "Old", NA))
    stats <- make_split_violin(pvm, "PLAUR_ALRA", "age_group", c("Young", "Old"), "ALRA-imputed PLAUR in FreshMG PVMs", 
        "ALRA-imputed PLAUR expression", file.path(outdir, "Figure_2Q_PLAUR_PVM_young_old.pdf"))
    old_pvm <- pvm %>% filter(age_group == "Old")
    stats <- bind_rows(stats, make_split_violin(old_pvm, "Myeloid_Inflammatory1", "PLAUR_status", c("PLAUR-", "PLAUR+"), 
        "Myeloid inflammatory signature in old FreshMG PVMs", "Seurat module score", file.path(outdir, "Figure_2R_old_PVM_myeloid_inflammatory.pdf")))
