# FreshMG human brain myeloid cells
...
input_file <- file.path(alex_root, "human/data/DataS1_FreshMG_final.h5ad")
...
saveRDS(obj, file.path(outdir, "FreshMG_seurat.rds"), compress = FALSE)
...
young <- unique(meta$age_plot[grepl("20|29|young", meta$age_plot, ignore.case = TRUE)])

old <- unique(meta$age_plot[grepl("90|old", meta$age_plot, ignore.case = TRUE)])

if (!length(young) || !length(old)) stop("FreshMG young/old labels were not recognized")

meta$age_group <- ifelse(meta$age_plot %in% young, "Young", ifelse(meta$age_plot %in% old, "Old", NA))

pvm <- meta %>% filter(grepl("PVM", published_class, ignore.case = TRUE))

# Manuscript figures and cell-level Wilcoxon tests
stats <- make_split_violin(meta, "PLAUR_ALRA", "age_group", c("Young", "Old"), "ALRA-imputed PLAUR in FreshMG", "ALRA-imputed PLAUR expression",
  file.path(outdir, "Figure_S2P_PLAUR_all_cells_young_old.pdf"))

# Manuscript figures and cell-level Wilcoxon tests
stats <- bind_rows(stats, make_split_violin(pvm, "PLAUR_ALRA", "age_group", c("Young", "Old"), "ALRA-imputed PLAUR in FreshMG PVMs",
  "ALRA-imputed PLAUR expression", file.path(outdir, "Figure_2Q_PLAUR_PVM_young_old.pdf")))

old_pvm <- pvm %>% filter(age_group == "Old")

# Manuscript figures and cell-level Wilcoxon tests
stats <- bind_rows(stats, make_split_violin(old_pvm, "Myeloid_Inflammatory1", "PLAUR_status", c("PLAUR-", "PLAUR+"), "Myeloid inflammatory signature in old FreshMG PVMs",
  "Seurat module score", file.path(outdir, "Figure_2R_old_PVM_myeloid_inflammatory.pdf")))
...
write_csv(deg, file.path(file.path(outdir, "DEG"), paste0("Figure_S2Q_PLAURplus_vs_PLAURminus", "_all_genes.csv")))

write_csv(filtered, file.path(file.path(outdir, "DEG"), paste0("Figure_S2Q_PLAURplus_vs_PLAURminus", "_filtered_for_enrichr.csv")))
...
if (length(results)) {
  write.xlsx(results, file.path(file.path(outdir, "DEG"), paste0("Figure_S2Q_PLAURplus_vs_PLAURminus", "_enrichr.xlsx")),
    overwrite = TRUE)
  ...
  if (nrow(plot_data)) {
    ...
    save_pdf(p, file.path(file.path(outdir, "DEG"), paste0("Figure_S2Q_PLAURplus_vs_PLAURminus", "_enrichr.pdf")), 10,
      9)
    write_csv(plot_data %>% mutate(term_plot = as.character(term_plot)), file.path(file.path(outdir, "DEG"), paste0("Figure_S2Q_PLAURplus_vs_PLAURminus",
