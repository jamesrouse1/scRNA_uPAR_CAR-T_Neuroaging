...
run_bone_marrow <- function() {
    ...
    samples <- data.frame(sample = ids, path = file.path(alex_root, "dataset3/counts/bone_marrow", ids, "filtered_feature_bc_matrix"), 
        age = ifelse(grepl("_Y_", ids), "Young", "Old"), treatment = ifelse(grepl("uPAR", ids), "uPAR", "UT"), 
    ...
    obj <- attach_labels(obj, file.path(alex_root, "dataset3/bone_marrow_updated_march/results/baccin_reference_transfer/baccin_reference_label_transfer_per_cell.csv"), 
    ...
    stats <- bind_rows(stats, make_split_violin(progenitors, "Chambers_Inflammaging1", "analysis_group", c("O UT", 
        "O uPAR"), "Chambers inflammaging signature in HSC/MPP", "Seurat module score", file.path(outdir, "Figure_3U_Chambers_HSC_MPP.pdf"), 
        c("#08306B", "#FF4902")), make_split_violin(progenitors, "Kovtonyuk_Myeloid_Bias1", "analysis_group", c("O UT", 
        "O uPAR"), "Kovtonyuk myeloid-bias signature in HSC/MPP", "Seurat module score", file.path(outdir, "Figure_3V_Kovtonyuk_HSC_MPP.pdf"), 
        c("#08306B", "#FF4902")))
    gmps <- obj@meta.data %>% mutate(cell = rownames(.)) %>% filter(grepl("GMP|Gran/Mono prog", cell_type, ignore.case = TRUE))
    stats <- bind_rows(stats, make_split_violin(gmps, "Chambers_Inflammaging1", "analysis_group", c("O UT", "O uPAR"), 
        "Chambers inflammaging signature in GMPs", "Seurat module score", file.path(outdir, "Figure_3W_Chambers_GMP.pdf"), 
        c("#08306B", "#FF4902")), make_split_violin(gmps, "Kovtonyuk_Myeloid_Bias1", "analysis_group", c("O UT", 
        "O uPAR"), "Kovtonyuk myeloid-bias signature in GMPs", "Seurat module score", file.path(outdir, "Figure_3X_Kovtonyuk_GMP.pdf"), 
        c("#08306B", "#FF4902")))
