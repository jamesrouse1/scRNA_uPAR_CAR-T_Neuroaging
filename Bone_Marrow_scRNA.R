...
ids <- c("Amor_AH05_Y_UT_1_M", "Amor_AH05_Y_UT_2_M", "Amor_AH05_Y_uPAR_1_M", "Amor_AH05_Y_uPAR_2_M", "Amor_AH06_O_UT_1_M",
  "Amor_AH06_O_UT_2_M", "Amor_AH06_O_uPAR_1_M", "Amor_AH06_O_uPAR_2_M")

# Sample information and filtered 10x matrices
samples <- data.frame(sample = ids, path = file.path(alex_root, "dataset3/counts/bone_marrow", ids, "filtered_feature_bc_matrix"),
  age = ifelse(grepl("_Y_", ids), "Young", "Old"), treatment = ifelse(grepl("uPAR", ids), "uPAR", "UT"), replicate = sub(".*_([12])_M$",
    "\\1", ids))
...
keep <- obj$percent.mt < 10 & obj$nCount_RNA > 500 & obj$nCount_RNA < 60000 & obj$nFeature_RNA > 250 & obj$nFeature_RNA <
  6000
...
obj <- FindNeighbors(obj, dims = 1:20, reduction = "integrated.dr", verbose = FALSE)

obj <- FindClusters(obj, resolution = 0.5, verbose = FALSE)

obj <- RunUMAP(obj, dims = 1:20, reduction = "integrated.dr", n.neighbors = 150, min.dist = 0.5, seed.use = 1, verbose = FALSE)

labels <- fread(file.path(alex_root, "dataset3", "bone_marrow_updated_march", "results", "baccin_reference_transfer", "baccin_reference_label_transfer_per_cell.csv"),
  data.table = FALSE)

idx <- match(colnames(obj), labels$cell_id)

if (anyNA(idx)) stop(sum(is.na(idx)), " cells are absent from ", file.path(alex_root, "dataset3", "bone_marrow_updated_march",
  "results", "baccin_reference_transfer", "baccin_reference_label_transfer_per_cell.csv"))

for (field in "baccin_reference_label") obj[[field]] <- labels[[field]][idx]

obj <- obj

obj$cell_type <- obj$baccin_reference_label
...
stats <- make_split_violin(monocytes, "Chambers_Inflammaging1", "analysis_group", c("O UT", "O uPAR"), "Chambers inflammaging signature in monocytes",
  "Seurat module score", file.path(outdir, "Figure_3I_Chambers_monocytes.pdf"), c("#08306B", "#FF4902"))

progenitors <- obj@meta.data %>% mutate(cell = rownames(.)) %>% filter(grepl("HSC|MPP|LMPP", cell_type, ignore.case = TRUE))

# Manuscript figures and cell-level Wilcoxon tests
stats <- bind_rows(stats, make_split_violin(progenitors, "Chambers_Inflammaging1", "analysis_group", c("O UT", "O uPAR"),
  "Chambers inflammaging signature in HSC/MPP", "Seurat module score", file.path(outdir, "Figure_3U_Chambers_HSC_MPP.pdf"),
  c("#08306B", "#FF4902")), make_split_violin(progenitors, "Kovtonyuk_Myeloid_Bias1", "analysis_group", c("O UT", "O uPAR"),
  "Kovtonyuk myeloid-bias signature in HSC/MPP", "Seurat module score", file.path(outdir, "Figure_3V_Kovtonyuk_HSC_MPP.pdf"),
  c("#08306B", "#FF4902")))

gmps <- obj@meta.data %>% mutate(cell = rownames(.)) %>% filter(grepl("GMP|Gran/Mono prog", cell_type, ignore.case = TRUE))

# Manuscript figures and cell-level Wilcoxon tests
stats <- bind_rows(stats, make_split_violin(gmps, "Chambers_Inflammaging1", "analysis_group", c("O UT", "O uPAR"), "Chambers inflammaging signature in GMPs",
  "Seurat module score", file.path(outdir, "Figure_3W_Chambers_GMP.pdf"), c("#08306B", "#FF4902")), make_split_violin(gmps,
  "Kovtonyuk_Myeloid_Bias1", "analysis_group", c("O UT", "O uPAR"), "Kovtonyuk myeloid-bias signature in GMPs", "Seurat module score",
