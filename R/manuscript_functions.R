...
support_dir <- file.path(project_root, "resources", "annotations")
...
attach_labels <- function(obj, file, label_columns) {
  ...
  for (field in label_columns) obj[[field]] <- labels[[field]][idx]
  obj
}
...
signature_files <- list(
  myeloid_mouse = file.path(project_root, "resources", "signatures", "myeloid_inflammatory_mouse.txt"),
  myeloid_human = file.path(project_root, "resources", "signatures", "myeloid_inflammatory_human.txt")
)
...
run_sorted_brain <- function() {
  ...
  obj <- attach_labels(obj, file.path(support_dir, "sorted_brain_labels.csv.gz"),
                       c("cell_type", "cell_type_grouped"))
...
run_post_treatment_brain <- function() {
  ...
  obj <- attach_labels(obj, file.path(support_dir, "post_treatment_brain_labels.csv.gz"),
                       c("cell_type", "cell_type_grouped"))
...
run_bone_marrow <- function() {
  ...
  obj <- attach_labels(obj, file.path(support_dir, "bone_marrow_baccin_labels.csv.gz"), "cell_type")
  obj$analysis_group <- factor(paste(substr(obj$age, 1, 1), obj$treatment),
                               levels = c("Y UT", "Y uPAR", "O UT", "O uPAR"))
  obj <- score_object(join_and_normalize(obj), signatures_mouse)
...
validate_inputs <- function() {
  paths <- c(
    ...
    sorted_brain_labels = file.path(support_dir, "sorted_brain_labels.csv.gz"),
    post_treatment_brain_labels = file.path(support_dir, "post_treatment_brain_labels.csv.gz"),
    bone_marrow_labels = file.path(support_dir, "bone_marrow_baccin_labels.csv.gz"),
